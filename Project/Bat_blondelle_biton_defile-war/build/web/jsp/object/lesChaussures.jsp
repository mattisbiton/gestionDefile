<%-- 
    Document   : lesChaussures
    Created on : Jan 3, 2023, 1:56:42 PM
    Author     : matti
--%>
...
<html>
    <head>
        ...
        <meta charset="UTF-8">
        <title>Chaussures — gestionDefile</title>
    </head>
    <body>
        <c:choose>
        <c:when test="${!empty sessionScope.roleAccount }">
            <% Chaussures c = (Chaussures) request.getAttribute("chaussures"); %>
            ...
            <div class="content">
                <center><h1>Informations sur la paire de chaussures</h1></center>
                ...
                <div class="container">
                    ...
                    <div class="info">
                        <p><b>Nom :</b> <%= c.getNom()%></p><br>
                        <p><b>Pointure :</b> <%=c.getTaille()%></p><br>
                        <p><b>Couturier :</b> <a href="servletDefile?id=<%= c.getLeCouturier().getId() %>&action=infosCouturier"><%= c.getLeCouturier().getPrenom() %> <%= c.getLeCouturier().getNom() %></a></p><br>
                        <p><b>Prix d'achat :</b> <%= c.getPrixAchat() %></p><br>
                        <p><b>Prix de location :</b> <%= c.getPrixLoc() %></p><br>
                        <p><b>Prix total :</b> <% double bijouTotal = c.getPrixAchat() + c.getPrixLoc(); %>
                          <%=bijouTotal%></p><br>
                    </div>
                </div>
            </div>
